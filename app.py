...
# Header
st.markdown(
    '<div class="header">💼 Employee Salary Predictor</div>', unsafe_allow_html=True
)
...
# Form title
st.markdown(
    '<div class="subheader">📝 Enter Employee Details</div>', unsafe_allow_html=True
)
...
with st.form("salary_form"):
    ...
    education_level = st.selectbox(
        "Education Level", options=label_encoders["Education Level"].classes_
    )
    job_title = st.selectbox("Job Title", options=label_encoders["Job Title"].classes_)
    years_of_experience = st.number_input(
        "Years of Experience", min_value=0, max_value=40, value=2
    )
...
st.markdown(
    """
<div style='text-align:center; margin-top:40px; font-size:14px; color:#9ca3af;'>
    Developed by Aman Verma - amanverma30072@gmail.com
</div>
""",
    unsafe_allow_html=True,
)
